from YOLOLITE.model.common import *
# from YOLOLITE.experimental import *
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s_dict', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            m.stride = torch.tensor([256 / x.shape[-2] for x in self.forward(torch.zeros(1, ch, 256, 256))])  # forward
...
# --------------------------repvgg & shuffle refuse---------------------------------
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            if isinstance(m, CBH) and hasattr(m, 'bn'):
                m.conv = fuse_conv_and_bn(m.conv, m.bn)  # update conv
                delattr(m, 'bn')  # remove batchnorm
                m.forward = m.fuseforward  # update forward

            if isinstance(m, Shuffle_Block):
...
def parse_model(d, ch):  # todo 包含网络结构配置文件的model_dict, input_channels(3)
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = len(anchors[0]) // 2
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5) todo 对每一个空间位置的检测结果 vector(255)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        n = max(round(n * gd), 1) if n > 1 else n  # depth gain; print
        if m in {Conv,  conv_bn_relu_maxpool, C3, Shuffle_Block}:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', action='store_true', help='test all YOLOLITE*.yaml')
    opt = parser.parse_args()

    with open('v5Lite-s.yaml') as f:
        dic = yaml.load(f, Loader=yaml.SafeLoader)

    model = Model(dic, ch=3, nc=20).cuda()
    y = model(torch.zeros(size=[4, 3, 512, 512]).cuda())
